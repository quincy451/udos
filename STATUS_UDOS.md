...
- added release-style boot packaging for VICE:
  ...
  - `make vice-action-alink` now uses the release image with deterministic
    ...
    `OBJ/*.OBJ`.
  - `make vice-action-alink-prg-matrix` now enumerates 1380 direct-PRG
    ...
    `143` from direct PRGs. Pass N is 7,109 bytes with 1,083 bytes free under
    ...
    direct PRGs print `43` and `154`; the 7,112-byte overlay retains 1,080 bytes
    ...
    direct PRGs print `33` and `154`; the 7,136-byte overlay retains 1,056 bytes
    ...
    6502 path matches. The 7,140-byte overlay retains 1,052 bytes under the
    ...
    binary32 4.0 and 3.0; Idun's generated-6502 path matches. The 7,323-byte
    overlay retains 869 bytes under a dedicated 768-byte gate.
    ...
    7.0; Idun's generated-6502 path matches. The 7,817-byte overlay retains 375
    ...
    Idun's generated-6502 path matches. The 8,136-byte overlay retains 56 bytes
