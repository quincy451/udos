...
- added release-style boot packaging for VICE:
  ...
  - `make vice-action-alink` now uses the release image with deterministic
    ...
    `OBJ/*.OBJ`.
  - `make vice-action-alink-prg-matrix` now enumerates 1379 direct-PRG
    ...
    Pass K is 5,899 bytes with 2,293 bytes free in its 8 KiB window.
    ...
    `143` from direct PRGs. Pass N is 7,131 bytes with 1,061 bytes free under
    ...
    direct PRGs print `43` and `154`; the 7,134-byte overlay retains 1,058 bytes
    ...
    direct PRGs print `33` and `154`; the 7,158-byte overlay retains 1,034 bytes
    ...
    6502 path matches. The 7,162-byte overlay retains 1,030 bytes under the
    ...
    binary32 4.0 and 3.0; Idun's generated-6502 path matches. The 7,345-byte
    overlay retains 847 bytes under a dedicated 768-byte gate.
    ...
    7.0; Idun's generated-6502 path matches. The 7,839-byte overlay retains 353
    ...
    Idun's generated-6502 path matches. The 8,158-byte overlay retains 34 bytes
