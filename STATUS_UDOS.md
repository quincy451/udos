...
- added release-style boot packaging for VICE:
  ...
  - `make vice-action-alink` now uses the release image with deterministic
    ...
    `OBJ/*.OBJ`.
  - `make vice-action-alink-prg-matrix` now enumerates 1372 direct-PRG
    ...
    `143` from direct PRGs. Pass N is 7,089 bytes with 1,103 bytes free under
    ...
    direct PRGs print `43` and `154`; the 7,092-byte overlay retains 1,100 bytes
    ...
    direct PRGs print `33` and `154`; the 7,116-byte overlay retains 1,076 bytes
    ...
    6502 path matches. The 7,120-byte overlay retains 1,072 bytes under the
    ...
    binary32 4.0 and 3.0; Idun's generated-6502 path matches. The 7,303-byte
    overlay retains 889 bytes under a dedicated 768-byte gate.
    ...
    7.0; Idun's generated-6502 path matches. The 7,797-byte overlay retains 395
    ...
    Idun's generated-6502 path matches. The 8,116-byte overlay retains 76 bytes
