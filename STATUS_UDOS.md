...
- added release-style boot packaging for VICE:
  ...
  - `make vice-action-alink` now uses the release image with deterministic
    ...
    `OBJ/*.OBJ`.
  - `make vice-action-alink-prg-matrix` now enumerates 1368 direct-PRG
    ...
    `143` from direct PRGs. Pass N is 7,058 bytes with 1,134 bytes free under
    ...
    direct PRGs print `43` and `154`; the 7,061-byte overlay retains 1,131 bytes
    ...
    direct PRGs print `33` and `154`; the 7,085-byte overlay retains 1,107 bytes
    ...
    6502 path matches. The 7,089-byte overlay retains 1,103 bytes under the
    ...
    binary32 4.0 and 3.0; Idun's generated-6502 path matches. The 7,272-byte
    overlay retains 920 bytes under a dedicated 768-byte gate.
    ...
    7.0; Idun's generated-6502 path matches. The 7,766-byte overlay retains 426
    ...
    Idun's generated-6502 path matches. The 8,085-byte overlay retains 107 bytes
