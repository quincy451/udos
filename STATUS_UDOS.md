...
- added release-style boot packaging for VICE:
  ...
  - `make vice-action-alink` now uses the release image with deterministic
    ...
    `OBJ/*.OBJ`.
  - `make vice-action-alink-prg-matrix` now enumerates 1373 direct-PRG
    ...
    `143` from direct PRGs. Pass N is 7,096 bytes with 1,096 bytes free under
    ...
    direct PRGs print `43` and `154`; the 7,099-byte overlay retains 1,093 bytes
    ...
    direct PRGs print `33` and `154`; the 7,123-byte overlay retains 1,069 bytes
    ...
    6502 path matches. The 7,127-byte overlay retains 1,065 bytes under the
    ...
    binary32 4.0 and 3.0; Idun's generated-6502 path matches. The 7,310-byte
    overlay retains 882 bytes under a dedicated 768-byte gate.
    ...
    7.0; Idun's generated-6502 path matches. The 7,804-byte overlay retains 388
    ...
    Idun's generated-6502 path matches. The 8,123-byte overlay retains 69 bytes
