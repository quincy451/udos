...
Current status docs report the broad ALINK direct-PRG matrix at 1386 shapes,
the non-runtime source-backed ACTC object-emission matrix at 196 shapes, and
the live MATH1/runtime launch matrix at 73 shapes. Treat those matrix counts as
status facts to update whenever the probe tables change.
...
introduced. Pass 7 is 7,371 bytes with 821 bytes free in its 8 KiB window.
...
Pass L is 6,142 bytes with 2,050 bytes free. Recursive/reentrant frames,
...
is 7,010 bytes with 1,182 bytes free under its 1 KiB gate, while pass L retains
...
inner false, and outer false paths. Pass N is 7,132 bytes with 1,060 bytes free
...
`RT_I_TO_F`, and `RT_PRINT_F` closure. Pass O is
7,135 bytes with 1,057 bytes free under the same 1 KiB gate. Loops, early
returns, controls beyond four,
...
covering early true/else exits and the fallback path. Pass P is
7,159 bytes with 1,033 bytes free under the 1 KiB gate. Loops, controls beyond
four or
...
generated-6502 path produces the same values. Pass Q is
7,163 bytes with 1,029 bytes free under the 1 KiB gate. Plain infinite `DO`,
loop `EXIT`, mixed
...
Pass R is 7,346 bytes with 846 bytes free under a dedicated 768-byte gate.
...
the same values. Pass S is 7,840 bytes with 352 bytes free under a dedicated
...
Pass T is 8,159 bytes with 33 bytes free under a dedicated 32-byte gate.
...
produce `-1`, `0`, and `1`. Pass U is 7,485 bytes with 707 bytes free under a
dedicated 640-byte gate; passes L through T retain their respective capacity
gates.
`DegToRad`, `RadToDeg`, `FPow`, `FSin`, `FCos`, `FTan`, `FATan`, `FATan2`,
`FASin`, `FACos`, `FSec`, `FCsc`, `FCot`, `FASec`, `FACsc`, `FACot`, and
`FVersin` are separately selected OBJ modules; the remaining public MATH1 gap
is 7 routines, beginning with `FHaversin`.
...
static exact-byte coverage, and 320 complex compiled-runtime cases use an
...
VICE. Pass K is 5,899 bytes with 2,293 bytes free. Native unary lowering now also recognizes
...
The 493-byte `RT_F_ATAN2.OBJ` dependency root implements signed-zero,
zero-axis, infinity, NaN, and all four quadrant rules. It imports only
`RT_F_DIV.OBJ`, `RT_F_ATAN.OBJ`, `RT_F_ADD.OBJ`, and `RT_F_SUB.OBJ`, allows
the destination to alias either source, and the focused direct PRG prints
`0.785398...` for `FATan2(1,1)`.
The 220-byte `RT_F_ASIN.OBJ` dependency root rejects NaN and magnitudes above
one with canonical quiet NaN, preserves signed zero, and produces signed
binary32 pi/2 at the endpoints. It imports only multiplication, subtraction,
square root, and two-argument arctangent. The focused direct PRG prints
`1.570796...` for `FASin(1)` while proving unrelated MATH1 roots remain absent.
The 71-byte `RT_F_ACOS.OBJ` dependency root implements source-level
`pi/2-FASin(x)` binary32 behavior, preserves exact endpoint results, and is
safe when its destination aliases its input. It imports only `RT_F_ASIN.OBJ`
and `RT_F_SUB.OBJ`; their transitive closure is 4,627 linked bytes. The
focused direct PRG prints `3.141592...` for `FACos(-1)` while proving
unrelated MATH1 roots remain absent.
The 71-byte `RT_F_SEC.OBJ` dependency root evaluates FCos into private storage,
divides binary32 one by that result, and remains safe when its destination
aliases its input. It imports only `RT_F_COS.OBJ` and `RT_F_DIV.OBJ`; their
transitive closure is 3,699 linked bytes. The focused direct PRG prints
`-2.402998...` for `FSec(2)` while proving sine, tangent, and power roots remain
absent.
The 71-byte `RT_F_CSC.OBJ` dependency root evaluates FSin into private storage,
divides binary32 one by that result, and remains safe when its destination
aliases its input. It imports only `RT_F_SIN.OBJ` and `RT_F_DIV.OBJ`; their
transitive closure is 3,676 linked bytes. The focused direct PRG prints
`1.099750...` for `FCsc(2)` while proving cosine, tangent, and power roots remain
absent.
The 113-byte `RT_F_COT.OBJ` dependency root preserves the source, evaluates
FCos and then FSin into separate private cells, and divides the binary32
results. It imports only `RT_F_COS.OBJ`, `RT_F_SIN.OBJ`, and `RT_F_DIV.OBJ`;
their transitive closure is 4,327 linked bytes. The focused direct PRG prints
`-0.457657...` for `FCot(2)` while proving tangent, secant, cosecant, and power
wrapper roots remain absent.
The 96-byte `RT_F_ASEC.OBJ` dependency root snapshots its source and evaluates
binary32 `FACos(1.0/value)` through private storage. It imports only
`RT_F_DIV.OBJ` and `RT_F_ACOS.OBJ`; their transitive closure is 4,723 linked
bytes. The focused direct PRG prints `1.047197...` for `FASec(2)` while proving
comparison, tangent, secant, cosecant, cotangent, and power wrapper roots remain
absent.
The 96-byte `RT_F_ACSC.OBJ` dependency root snapshots its source and evaluates
binary32 `FASin(1.0/value)` through private storage. It imports only
`RT_F_DIV.OBJ` and `RT_F_ASIN.OBJ`; their transitive closure is 4,652 linked
bytes. The focused direct PRG prints `0.523598...` for `FACsc(2)` while proving
inverse cosine, comparison, and unrelated trigonometric wrapper roots remain
absent.
The 28-byte `RT_F_ACOT.OBJ` dependency root copies its source pointer into the
`FATan2` x slot, points the y slot at embedded binary32 one, and evaluates
source-defined `FATan2(1.0,value)`. It imports only `RT_F_ATAN2.OBJ`; its
transitive closure is 3,858 linked bytes and remains safe when destination and
source alias. The focused direct PRG prints `0.785398...` for `FACot(1)` while
proving division and unrelated trigonometric wrapper roots remain absent.
The 71-byte `RT_F_VERSIN.OBJ` dependency root snapshots its source, evaluates
binary32 `FCos(value)` through private storage, and subtracts that result from
embedded binary32 one. It directly imports only `RT_F_COS.OBJ` and
`RT_F_SUB.OBJ`; its transitive closure is 3,699 linked bytes and remains safe
when destination and source alias. The focused direct PRG prints `0.459697...`
for `FVersin(1)` while proving unrelated trigonometric wrapper roots remain
absent.
The 71-byte `RT_F_HAVERSIN.OBJ` dependency root snapshots its source, evaluates
binary32 `FVersin(value)` through private storage, and divides that result by
embedded binary32 two. It directly imports only `RT_F_VERSIN.OBJ` and
`RT_F_DIV.OBJ`; its transitive closure is 3,770 linked bytes and remains safe
when destination and source alias. The focused direct PRG prints `0.229848...`
for `FHaversin(1)` while proving sibling wrapper roots remain absent.
...
exponential/natural-logarithm/trigonometric, inverse-secant, inverse-cosecant,
inverse-cotangent, versed-sine, and haversine dependencies, and
REAL printing
while proving sibling REAL helpers remain absent. Pass 6 is
8,095 bytes with 97 bytes free under its enforced reserve. General REAL expression
trees and the remaining 6
MATH1 routines are
still compiler work.
