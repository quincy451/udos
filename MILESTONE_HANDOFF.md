...
Current status docs report the broad ALINK direct-PRG matrix at 1382 shapes and
...
introduced. Pass 7 is 7,229 bytes with 963 bytes free in its 8 KiB window.
...
Pass L is 6,132 bytes with 2,060 bytes free. Recursive/reentrant frames,
...
is 7,000 bytes with 1,192 bytes free under its 1 KiB gate, while pass L retains
...
inner false, and outer false paths. Pass N is 7,122 bytes with 1,070 bytes free
...
`RT_I_TO_F`, and `RT_PRINT_F` closure. Pass O is 7,118 bytes with 1,074 bytes
...
covering early true/else exits and the fallback path. Pass P is 7,149 bytes with 1,043 bytes free under the 1 KiB gate. Loops, controls beyond four or
...
generated-6502 path produces the same values. Pass Q is 7,146 bytes with 1,046
...
Pass R is 7,336 bytes with 856 bytes free under a dedicated 768-byte gate.
...
the same values. Pass S is 7,830 bytes with 362 bytes free under a dedicated
...
Pass T is 8,149 bytes with 43 bytes free under a dedicated 32-byte gate.
...
produce `-1`, `0`, and `1`. Pass U is 7,477 bytes with 715 bytes free under a
...
`FASin`, `FACos`, `FSec`, `FCsc`, `FCot`, and `FASec` are separately selected
OBJ modules; the remaining public MATH1 gap is 10 routines, beginning with
`FACsc`.
...
static exact-byte coverage, and 316 complex compiled-runtime cases use an
...
8,094 bytes with 98 bytes free under its enforced reserve. General REAL expression
trees and the remaining 10
MATH1 routines are
still compiler work.
