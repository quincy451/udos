...
Current status docs report the broad ALINK direct-PRG matrix at 1370 shapes and
...
introduced. Pass 7 is 6,840 bytes with 1,352 bytes free in its 8 KiB window.
...
Pass L is 6,084 bytes with 2,108 bytes free. Recursive/reentrant frames,
...
is 6,953 bytes with 1,239 bytes free under its 1 KiB gate, while pass L retains
...
inner false, and outer false paths. Pass N is 7,075 bytes with 1,117 bytes free
...
`RT_I_TO_F`, and `RT_PRINT_F` closure. Pass O is 7,078 bytes with 1,114 bytes
...
covering early true/else exits and the fallback path. Pass P is 7,102 bytes
with 1,090 bytes free under the 1 KiB gate. Loops, controls beyond four or
...
generated-6502 path produces the same values. Pass Q is 7,106 bytes with 1,086
...
Pass R is 7,289 bytes with 903 bytes free under a dedicated 768-byte gate.
...
the same values. Pass S is 7,783 bytes with 409 bytes free under a dedicated
...
Pass T is 8,102 bytes with 90 bytes free under a dedicated 32-byte gate.
...
produce `-1`, `0`, and `1`. Pass U is 7,432 bytes with 760 bytes free under a
...
public MATH1 gap is 22 routines.
...
static exact-byte coverage, and 304 complex compiled-runtime cases use an
