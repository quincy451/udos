...
Current status docs report the broad ALINK direct-PRG matrix at 1379 shapes and
...
introduced. Pass 7 is 7,129 bytes with 1,063 bytes free in its 8 KiB window.
...
Pass L is 6,140 bytes with 2,052 bytes free. Recursive/reentrant frames,
...
is 7,009 bytes with 1,183 bytes free under its 1 KiB gate, while pass L retains
...
inner false, and outer false paths. Pass N is 7,131 bytes with 1,061 bytes free
...
`RT_I_TO_F`, and `RT_PRINT_F` closure. Pass O is 7,134 bytes with 1,058 bytes
...
covering early true/else exits and the fallback path. Pass P is 7,158 bytes with 1,034 bytes free under the 1 KiB gate. Loops, controls beyond four or
...
generated-6502 path produces the same values. Pass Q is 7,162 bytes with 1,030
...
Pass R is 7,345 bytes with 847 bytes free under a dedicated 768-byte gate.
...
the same values. Pass S is 7,839 bytes with 353 bytes free under a dedicated
...
Pass T is 8,158 bytes with 34 bytes free under a dedicated 32-byte gate.
...
`FASin`, `FACos`, and `FSec` are separately selected OBJ modules; the remaining
public MATH1 gap is 13 routines, beginning with `FCsc`.
...
static exact-byte coverage, and 313 complex compiled-runtime cases use an
...
8,084 bytes with 108 bytes free under its enforced reserve. General REAL expression
trees and the remaining 13
MATH1 routines are
still compiler work.
