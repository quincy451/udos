...
Current status docs report the broad ALINK direct-PRG matrix at 1380 shapes and
...
introduced. Pass 7 is 7,162 bytes with 1,030 bytes free in its 8 KiB window.
...
Pass L is 6,119 bytes with 2,073 bytes free. Recursive/reentrant frames,
...
is 6,987 bytes with 1,205 bytes free under its 1 KiB gate, while pass L retains
...
inner false, and outer false paths. Pass N is 7,109 bytes with 1,083 bytes free
...
`RT_I_TO_F`, and `RT_PRINT_F` closure. Pass O is 7,112 bytes with 1,080 bytes
...
covering early true/else exits and the fallback path. Pass P is 7,136 bytes with 1,056 bytes free under the 1 KiB gate. Loops, controls beyond four or
...
generated-6502 path produces the same values. Pass Q is 7,140 bytes with 1,052
...
Pass R is 7,323 bytes with 869 bytes free under a dedicated 768-byte gate.
...
the same values. Pass S is 7,817 bytes with 375 bytes free under a dedicated
...
Pass T is 8,136 bytes with 56 bytes free under a dedicated 32-byte gate.
...
produce `-1`, `0`, and `1`. Pass U is 7,464 bytes with 728 bytes free under a
...
`FASin`, `FACos`, `FSec`, and `FCsc` are separately selected OBJ modules; the
remaining public MATH1 gap is 12 routines, beginning with `FCot`.
...
static exact-byte coverage, and 314 complex compiled-runtime cases use an
...
8,091 bytes with 101 bytes free under its enforced reserve. General REAL expression
trees and the remaining 12
MATH1 routines are
still compiler work.
