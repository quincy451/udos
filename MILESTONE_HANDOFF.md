...
Current status docs report the broad ALINK direct-PRG matrix at 1368 shapes and
...
introduced. Pass 7 is 6,801 bytes with 1,391 bytes free in its 8 KiB window.
...
Pass L is 6,067 bytes with 2,125 bytes free. Recursive/reentrant frames,
...
is 6,936 bytes with 1,256 bytes free under its 1 KiB gate, while pass L retains
...
inner false, and outer false paths. Pass N is 7,058 bytes with 1,134 bytes free
...
`RT_I_TO_F`, and `RT_PRINT_F` closure. Pass O is 7,061 bytes with 1,131 bytes
...
covering early true/else exits and the fallback path. Pass P is 7,085 bytes
with 1,107 bytes free under the 1 KiB gate. Loops, controls beyond four or
...
generated-6502 path produces the same values. Pass Q is 7,089 bytes with 1,103
...
Pass R is 7,272 bytes with 920 bytes free under a dedicated 768-byte gate.
...
the same values. Pass S is 7,766 bytes with 426 bytes free under a dedicated
...
Pass T is 8,085 bytes with 107 bytes free under a dedicated 32-byte gate.
...
produce `-1`, `0`, and `1`. Pass U is 7,415 bytes with 777 bytes free under a
...
public MATH1 gap is 24 routines.
...
static exact-byte coverage, and 302 complex compiled-runtime cases use an
...
truncation/floor/ceiling/rounding/fractional-part/remainder/hypotenuse/
exponential/natural-logarithm dependencies, and REAL printing
while proving sibling REAL helpers remain absent. Pass 6 is
8,077 bytes with 115 bytes free under its enforced 96-byte reserve. General REAL expression
trees and the remaining 24
MATH1 routines are
still compiler work.
