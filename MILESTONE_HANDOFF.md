...
Current status docs report the broad ALINK direct-PRG matrix at 1372 shapes and
...
introduced. Pass 7 is 6,896 bytes with 1,296 bytes free in its 8 KiB window.
...
Pass L is 6,098 bytes with 2,094 bytes free. Recursive/reentrant frames,
...
is 6,967 bytes with 1,225 bytes free under its 1 KiB gate, while pass L retains
...
inner false, and outer false paths. Pass N is 7,089 bytes with 1,103 bytes free
...
`RT_I_TO_F`, and `RT_PRINT_F` closure. Pass O is 7,092 bytes with 1,100 bytes
...
covering early true/else exits and the fallback path. Pass P is 7,116 bytes
with 1,076 bytes free under the 1 KiB gate. Loops, controls beyond four or
...
generated-6502 path produces the same values. Pass Q is 7,120 bytes with 1,072
...
Pass R is 7,303 bytes with 889 bytes free under a dedicated 768-byte gate.
...
the same values. Pass S is 7,797 bytes with 395 bytes free under a dedicated
...
Pass T is 8,116 bytes with 76 bytes free under a dedicated 32-byte gate.
...
produce `-1`, `0`, and `1`. Pass U is 7,446 bytes with 746 bytes free under a
dedicated 640-byte gate; passes L through T retain their respective capacity
gates.
`DegToRad`, `RadToDeg`, `FPow`, and `FSin` are separately selected OBJ modules;
the remaining public MATH1 gap is 20 routines.
...
static exact-byte coverage, and 306 complex compiled-runtime cases use an
...
exponential/natural-logarithm dependencies, and REAL printing
while proving sibling REAL helpers remain absent. Pass 6 is
8,096 bytes with exactly 96 bytes free under its enforced reserve. General REAL expression
trees and the remaining 20
MATH1 routines are
still compiler work.
