...
Current status docs report the broad ALINK direct-PRG matrix at 1371 shapes and
...
introduced. Pass 7 is 6,870 bytes with 1,322 bytes free in its 8 KiB window.
...
Pass L is 6,091 bytes with 2,101 bytes free. Recursive/reentrant frames,
...
is 6,960 bytes with 1,232 bytes free under its 1 KiB gate, while pass L retains
...
inner false, and outer false paths. Pass N is 7,082 bytes with 1,110 bytes free
...
`RT_I_TO_F`, and `RT_PRINT_F` closure. Pass O is 7,085 bytes with 1,107 bytes
...
covering early true/else exits and the fallback path. Pass P is 7,109 bytes
with 1,083 bytes free under the 1 KiB gate. Loops, controls beyond four or
...
generated-6502 path produces the same values. Pass Q is 7,113 bytes with 1,079
...
Pass R is 7,296 bytes with 896 bytes free under a dedicated 768-byte gate.
...
the same values. Pass S is 7,790 bytes with 402 bytes free under a dedicated
...
Pass T is 8,109 bytes with 83 bytes free under a dedicated 32-byte gate.
...
produce `-1`, `0`, and `1`. Pass U is 7,439 bytes with 753 bytes free under a
dedicated 640-byte gate; passes L through T retain their respective capacity
gates.
`DegToRad`, `RadToDeg`, and `FPow` are separately selected OBJ modules; the
remaining public MATH1 gap is 21 routines.
...
static exact-byte coverage, and 305 complex compiled-runtime cases use an
...
8,089 bytes with 103 bytes free under its enforced 96-byte reserve. General REAL expression
trees and the remaining 21
MATH1 routines are
still compiler work.
