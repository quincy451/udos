...
Current status docs report the broad ALINK direct-PRG matrix at 1383 shapes,
the non-runtime source-backed ACTC object-emission matrix at 196 shapes, and
the live MATH1/runtime launch matrix at 70 shapes. Treat those matrix counts as
...
introduced. Pass 7 is 7,260 bytes with 932 bytes free in its 8 KiB window.
...
Pass L is 6,139 bytes with 2,053 bytes free. Recursive/reentrant frames,
...
is 7,007 bytes with 1,185 bytes free under its 1 KiB gate, while pass L retains
...
inner false, and outer false paths. Pass N is 7,129 bytes with 1,063 bytes free
...
`RT_I_TO_F`, and `RT_PRINT_F` closure. Pass O is 7,132 bytes with 1,060 bytes
...
covering early true/else exits and the fallback path. Pass P is 7,156 bytes with 1,036 bytes free under the 1 KiB gate. Loops, controls beyond four or
...
generated-6502 path produces the same values. Pass Q is 7,160 bytes with 1,032
...
Pass R is 7,343 bytes with 849 bytes free under a dedicated 768-byte gate.
...
the same values. Pass S is 7,837 bytes with 355 bytes free under a dedicated
...
Pass T is 8,156 bytes with 36 bytes free under a dedicated 32-byte gate.
...
produce `-1`, `0`, and `1`. Pass U is 7,484 bytes with 708 bytes free under a
...
`FASin`, `FACos`, `FSec`, `FCsc`, `FCot`, `FASec`, and `FACsc` are separately
selected OBJ modules; the remaining public MATH1 gap is 9 routines, beginning
with `FACot`.
...
static exact-byte coverage, and 317 complex compiled-runtime cases use an
...
exponential/natural-logarithm/trigonometric and inverse-secant dependencies, and
REAL printing
