...
Current status docs report the broad ALINK direct-PRG matrix at 1381 shapes and
...
introduced. Pass 7 is 7,195 bytes with 997 bytes free in its 8 KiB window.
...
Pass L is 6,125 bytes with 2,067 bytes free. Recursive/reentrant frames,
...
is 6,993 bytes with 1,199 bytes free under its 1 KiB gate, while pass L retains
...
inner false, and outer false paths. Pass N is 7,115 bytes with 1,077 bytes free
...
covering early true/else exits and the fallback path. Pass P is 7,142 bytes with 1,050 bytes free under the 1 KiB gate. Loops, controls beyond four or
...
Pass R is 7,329 bytes with 863 bytes free under a dedicated 768-byte gate.
...
the same values. Pass S is 7,823 bytes with 369 bytes free under a dedicated
...
Pass T is 8,142 bytes with 50 bytes free under a dedicated 32-byte gate.
...
produce `-1`, `0`, and `1`. Pass U is 7,470 bytes with 722 bytes free under a
...
`FASin`, `FACos`, `FSec`, `FCsc`, and `FCot` are separately selected OBJ
modules; the remaining public MATH1 gap is 11 routines, beginning with `FASec`.
...
static exact-byte coverage, and 315 complex compiled-runtime cases use an
