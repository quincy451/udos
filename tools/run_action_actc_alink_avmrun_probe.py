...
def source_text() -> str:
    return (
        ...
        'PrintE("HELLO")\r'
        'W()\r'
        'PrintI(((100 + 2) * 2 - 80) <> 124)\r'
        'PrintIE(((60 - 15) / 3 + 42) <= 57)\r'
        'RETURN\r'
    )
...
def read_alink_trace(client: vp.BinaryMonitorClient) -> str:
    try:
        value = client.memory_get(ALINK_TRACE_ADDR, ALINK_TRACE_ADDR)[0]
        return f"ALINK_TRACE=0x{value:02X}"
    except Exception:
...
def run_once(image: Path, work_root: Path, project_name: str, connect_delay: float) -> None:
    ...
    try:
        ...
        while time.monotonic() < deadline:
            ...
            raise vp.ViceError(f"timed out waiting for ALINK OK ({read_alink_trace(client)}); last screen was:\n{screen}")
...
def main() -> int:
    ...
    work_root = fs_root.parent / f"{fs_root.name}-actc-alink-avmrun"
