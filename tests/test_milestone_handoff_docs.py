...
class MilestoneHandoffDocsTests(unittest.TestCase):
    def test_handoff_matches_current_project_boundary(self) -> None:
        ...
        self.assertIn("1368 shapes", handoff)
        self.assertIn("196 shapes", handoff)
        self.assertIn("302 complex compiled-runtime cases", handoff)
        ...
        self.assertIn("7,272 bytes with 920 bytes free", handoff)
        self.assertIn("real_function_for_postfix.act", handoff)
        self.assertIn("ACTC_OVLS.BIN", handoff)
        self.assertIn("7,766 bytes with 426 bytes free", handoff)
        ...
        self.assertIn("6,801 bytes with 1,391 bytes free", handoff)
        self.assertIn("6,067 bytes with 2,125 bytes free", handoff)
        self.assertIn("6,936 bytes with 1,256 bytes free", handoff)
        self.assertIn("7,058 bytes with 1,134 bytes free", handoff)
        self.assertIn("7,415 bytes with 777 bytes free", handoff)
        self.assertIn("remaining 24", handoff)
