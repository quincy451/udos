...
- `make vice-action-alink`
  ...
  - emits a deterministic `BIN/MAIN.MAP` dependency map as the first UDOS-native linker slice, including export, call, live, entry, payload, include, and resolve lines from the current object metadata, without assuming export slot `0` is the entry proc
