...
- `make vice-action-alink-prg-matrix`
  ...
  - currently enumerates 1354 object/link shapes
...
- `make vice-action-actc-alink-launch-object-emission-matrix`
  ...
  - currently enumerates all 186 non-runtime, non-object-code source-backed
  ...
  - includes bounded REAL-function control with one conditional, two sequential
    conditionals, and two conditionals nested to depth two; the latter cases
    verify independent internal labels, generic ALINK relocation, reachable-only
    helper closure, and direct-PRG results `43` and `143`
